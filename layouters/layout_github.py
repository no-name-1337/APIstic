...
def layout_github(df):
    print(df)
    df['Commit Date Year'] = pd.to_datetime(df['Commit Date']).dt.year
    ...
    container_overview.subheader('Dataset Overview')
    ...
    container_overview.dataframe(metrics_df)

    col1, col2 = container_overview.columns(2)
    with col1:
        # st.write('Number of Yearly Created APIs by OpenAPI Version')
        api_counts = df.groupby(['Openapi Version', 'Commit Date Year']).size().reset_index(name='Count')
        # Pivot the data to have years as the index and versions as columns filled with the counts
        pivot_df = api_counts.pivot(index='Commit Date Year', columns='Openapi Version', values='Count').fillna(0)
    ...
    with col2:
        ...
        grouped_df = df.groupby('Commit Date Year')[cols_to_sum].sum().reset_index()

        # Create the plot
        fig = px.bar(grouped_df, x='Commit Date Year', y=cols_to_sum, barmode='stack',
    ...
    for metric in selected_structure_metric:
        ...
        with col1:
            ...
            fig_box = px.box(df, x='Commit Date Year', y=metric)
            st.plotly_chart(fig_box, use_container_width=True)
        with col2:
            st.subheader(f'Distribution of {metric} over Years')
            fig_bar = px.bar(df, x='Commit Date Year', y=metric, barmode='group')
        ...
        with col3:
            if metric == 'parameterised_operations':
                ...
                fig_box = px.box(df, x='Commit Date Year', y='Used Parameters')
                st.plotly_chart(fig_box, use_container_width=True)
            else:
